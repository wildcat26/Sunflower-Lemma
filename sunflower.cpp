...
bool premise (int d, int k, int modf) { 	// returns iff True if |F| > d*d!*(k - 1)^d
	int ans = d;
	int cpy = d;
	while (d) {
		if (modf <= ans)
...
int main () {
	...
	do {
		...
		int l = -1;			// the vertex with |F_l| > d!*(k - 1)^d

		int premiseByD = premise(d, k, m)/d;
		cout << "premiseByD = " << premiseByD << "\n";
		for (int ii = 0; ii < n; ++ii) {
			int i = u[ii];
			if (subsetIndices[i].size() > premiseByD) {		// auxiliary lemma can now be applied on 'l'
				l = i;
				break;
			}
		}
		assert(l != -1);
		cout << "Computing a sunflower with " << k << " petals, l = " << l << "\n";

		vector <int> sunflowerIndices;
		vector <bool> excluded(n + 1, false);
		// input should only be subsets containing l
		// CAUTION - subsetIndices should NOW have indices of only sets being given as input

		// fix subsetIndices
		vector <int> setsContainingL;
		for (int idx : subsetIndices[l])
			setsContainingL.push_back(idx);

		for (int i = 0; i <= n; ++i)		// now clear subsetIndices
			subsetIndices[i].clear();
		
		for (int lsidx : setsContainingL) {
			for (int elem : subsets[lsidx])
				subsetIndices[elem].push_back(lsidx);
		}

		for (int elem = 1; elem <= n; ++elem)			// EXCLUDE elems NOT taken by sets containing 'l'
			if (0 == subsetIndices[elem].size())
				excluded[elem] = 1;

		sunflower(subsetIndices[l], excluded, k, subsetIndices[l].size(), sunflowerIndices);
